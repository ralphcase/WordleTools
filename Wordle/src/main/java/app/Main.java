...
//        Solver solver = new Solver(repo, hard, Solver.Mode.ALL);
        ...
        solver.applyFeedback(new Word("SANER"), Feedback.of(gray, yellow, yellow, gray, yellow));
        DictionaryBuilder.predictWordlebot(solver.remainingCandidates());

        solver.applyFeedback(new Word("DRANK"), Feedback.of(gray, green, green, yellow, gray));
//        solver.applyFeedback(new Word("ORATE"), Feedback.of(gray, green, green, yellow, green));
