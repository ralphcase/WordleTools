...
//        Solver solver = new Solver(repo, hard, Solver.Mode.ALL);
        ...
        solver.applyFeedback(new Word("ORATE"), Feedback.of(yellow, gray, gray, gray, green));
        DictionaryBuilder.predictWordlebot(solver.remainingCandidates());

        solver.applyFeedback(new Word("SPOIL"), Feedback.of(yellow, yellow, yellow, gray, gray));
//        solver.applyFeedback(new Word("CHESS"), Feedback.of(gray, gray, yellow, green, yellow));
